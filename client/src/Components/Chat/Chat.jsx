...
class Chat extends Component {
  ...
  searchChat = async () => {
    ...
    const res = await axios.get(
      `http://localhost:8080/chat-search?u1=${this.state.username}&u2=${this.state.to}&query=${this.state.searchQuery}`
    );

    if (res.status && res.data !== undefined) {
      this.setState({ chats: res.data.reverse() });
      this.renderChatHistory(this.state.username, res.data.reverse());
    } else {
      this.setState({ chatHistory: [] });
    }
  };
