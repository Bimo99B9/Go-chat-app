...
func CreateFetchChatBetweenIndex() {
	res, err := redisClient.Do(context.Background(),
		...
		"$.message", "AS", "message", "TEXT", // Add this line
	).Result()

	fmt.Println(res, err)
}

func SearchInChat(username1, username2, query string) ([]model.Chat, error) {

	searchQuery := fmt.Sprintf("@from:{%s|%s} @to:{%s|%s} @message:*%s*",
		username1, username2, username1, username2, query)
